...
// Based heavily on <https://github.com/esp-rs/esp-hal/tree/967c478846a501bfac2f54a3eade158586aed7d3/esp-hal-embassy/src/executor>
...
const THREAD_MODE_CONTEXT: u8 = 16;
...
///
/// This executor is capable of waking an executor running on another core if
/// work needs to be completed there for a task to progress on this core.
pub struct Executor {
    inner: raw::Executor,
...
impl Executor {
    ...
    ///
    /// This will use software-interrupt 3 which isn't available for anything
    /// else to wake the other core(s).
    pub fn new() -> Self {
        unsafe {
    ...
    pub fn run(&'static mut self, init: impl FnOnce(Spawner, &'static AtomicU32)) -> ! {
        init(self.inner.spawner(), &self.idle_cycles);
    ...
    #[doc(hidden)]
    pub fn wait_impl(&self, cpu: usize) {
        ...
        let idle_start = get_cycle_count();
        ...
        if SIGNAL_WORK_THREAD_MODE[cpu].load(Ordering::SeqCst) {
            ...
            // Not in the critical section but the results Look Right™ so it works for now
            let idle_end = get_cycle_count();
            self.idle_cycles
                .add(idle_end.wrapping_sub(idle_start), Ordering::AcqRel);
        }
    }
}
